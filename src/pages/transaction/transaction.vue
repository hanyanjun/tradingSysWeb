<template>
  <div style="background: #F0F2F5;">
      ...
      <a-form :form="form"
        ...
        <a-form-item label="交易对"
          ...
          <a-dropdown :trigger="['click']"
                      :overlayStyle="{'max-height':'200px','overflow-y':'scroll','background':'white','border-bottom':'1px solid #e8e8e8'}">
            ...
            <div slot="overlay">
              <a-list bordered
                ...
                <a-list-item slot="renderItem"
                             ...
                             @click.stop="setFormValue('money',item)">{{item}}</a-list-item>
    ...
    <a-card title="现货交易"
          ...
          <a-table :columns="columns"
            ...
            <span slot="price"
              ...
              <span class="green pointer"
                    @click.stop="setPrice('form2',text)">{{text}}</span>
            </span>
            <template slot="title"
                      slot-scope="currentPageData">
              <span>
                <a-icon type="fall"
                        style="margin-right:8px" />买单(实时刷新)</span>
            </template>
          </a-table>
        </a-col>
        <a-col :xs='24'
          ...
          <a-table :columns="columns"
            ...
            <span slot="price"
              ...
              <span class="red pointer"
                    @click.stop="setPrice('form1',text)">{{text}}</span>
            </span>
            <template slot="title"
                      slot-scope="currentPageData">
              <span>
                <a-icon type="rise"
                        style="margin-right:8px" />卖单(实时刷新)</span>
            </template>
          </a-table>
        </a-col>
      </a-row>
      <a-row type="flex"
        ...
        <a-col :xs='24'
               ...
               :lg="11"
               :xl="11">
          <a-card style="text-aligin:left"
                  class="card_head_small"
            ...
            <a-row slot="title"
              ...
              <span style="color:#666;font-size:13px">
                <a-icon type="wallet" />&nbsp;&nbsp;&nbsp;{{available[1]}} {{currency[1]}}</span>
            </a-row>
            <a-form :form="form1">
              ...
              <a-form-item label="数量"
                ...
                <a-radio-group @change="PurchaseSumSelect('change')"
                               v-model="sum1"
                               size="small"
                               buttonStyle="solid">
                  <a-radio-button :key="item.value"
                                  :value="item.value"
                                  @click="PurchaseSumSelect"
                                  v-for="item in options.arr">{{item.label}}</a-radio-button>
                </a-radio-group>
              </a-form-item>
              ...
              <a-form-item label="金额"
                ...
                <a-input disabled
                         placeholder="自动计算"
                         v-decorator="['prices']" /><span style="margin-left:8px">({{currency[1]}})</span>
              </a-form-item>
        ...
        <a-col :xs='24'
          ...
          <a-card style="text-aligin:left"
            ...
            <a-row slot="title"
              ...
              <span style="color:#666;font-size:13px">
                <a-icon type="wallet" />&nbsp;&nbsp;&nbsp;{{available[0]}} {{currency[0]}}</span>
            </a-row>
            <a-form :form="form2">
              <a-form-item label="交易所客户号"
                           :label-col="{ span: 6 }"
              ...
              <a-form-item label="数量"
                ...
                <a-radio-group @change="sellOutSelect('change')"
                               v-model="sum2"
                               size="small"
                               buttonStyle="solid">
                  <a-radio-button :key="item.value"
                                  ...
                                  @click="sellOutSelect">{{item.label}}</a-radio-button>
                </a-radio-group>
              </a-form-item>
              <template v-if="accInfo.info">
                <span>
    ...
    <a-card :bordered="false"
      ...
      <a-table :columns="columes1"
               :dataSource="hisArr"
               :scroll="{ y: 600 }"
               :locale="{emptyText : '暂无数据'}"
               :pagination="{pageSize : 10 }">
        <span slot="Ts"
              slot-scope="text">{{$time(new Date(text),'yyyy-MM-dd HH:mm:ss')}}</span>
        <span slot="Type"
              slot-scope="text">
        ...
        <span slot="action"
          ...
          <template v-if="record.Status == '0' || record.Status == '3'">
            <a-popconfirm title="确定撤销该订单？"
                          @confirm="cancleOrder(record)"
                          okText="Yes"
                          cancelText="No">
...
export default {
  ...
  computed: {
    ...
    table() {
      ...
      var a11 = [],
        a22 = [];
      a1.forEach((v, i) => {
        a11.push({ index: i, price: v, sum: a2[i], key: i });
      });
      a3.forEach((v, i) => {
        a22.push({ index: i, price: v, sum: a2[i], key: i });
      });
  ...
  created() {
    ...
    let arr = JSON.parse(
      JSON.stringify(this.$store.state.selectData.hisTaskTable)
    );
    arr.push({
      title: "操作",
      dataIndex: "action",
  ...
  methods: {
    gainValue(item) {
      ...
      if (this.filterSymbol.length == 0) {
        this.$message.error(
          `${this.marketInfo.market}下未找到交易对：${symbol}，订阅行情失败！`
        );
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          if (this.filterSymbol.length != 0) {
            ...
            this.$message.error(
              `${this.marketInfo.market}下未找到交易对：${
                values.money
              }，订阅行情失败！`
            );
            this.$store.dispatch("subscribeMarket", { onlyCancle: true });
    ...
    PurchaseSumSelect(type) {
      ...
      this.form1.setFieldsValue({
        ...
        sum: this.$utils.mul(
          this.$utils.div(
            this.accInfo.info.info[arr[1]].Available,
            values.price
          ),
          this.sum1
        )
    ...
    Purchase() {
      this.form1.validateFields((err, values) => {
        if (!err) {
          ...
          if (this.available[1] < values.sum) {
            this.$confirm({
              title: "当前下单量小于持仓量，是否仍然下单？",
              onOk() {
                add.call(this);
              },
              onCancel() {}
    ...
    selectHisTime(e) {
      ...
      this.$api
        .getOrders({
          accountid: this.hisForm.accountid,
          start: t1,
          end: t,
          symbol: this.marketInfo.currency.replace("-", ""),
          exchange: this.marketInfo.market
        })
        .then(obj => {
          let arr1 = [];
          if (obj.data) {
          ...
          this.$store.state.orderMsgInfo = Object.assign(
            {},
            this.$store.state.orderMsgInfo
          );
    ...
    sellOut() {
      this.form2.validateFields((err, values) => {
        if (!err) {
          ...
          let values = this.form2.getFieldsValue();
          if (this.available[0] < values.sum) {
            this.$confirm({
              title: "当前下单量小于持仓量，是否仍然下单？",
              onOk() {
                add.call(this);
    ...
    detailPrices(type) {
      ...
      if (
        values.price &&
        values.sum &&
        /((\d+$)|(\d+[\.]+\d+$))/g.test(values.price) &&
        /((\d+$)|(\d+[\.]+\d+$))/g.test(values.sum)
      ) {
        this[type].setFieldsValue({
